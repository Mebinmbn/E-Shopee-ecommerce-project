    ...
    try {
      ...
      if (existingColor) {
        ...
        return res.status(400).json({
          success: false,
          message: "Color name or hex already exists",
        });
      }
      ...
      if (color && colorHex) {
        ...
        res
          .status(400)
          .json({ success: false, message: "Missing color name or hex" });
      }
    } catch (error) {
      console.log(error);
  ...
  addSize: async (req, res) => {
    ...
    try {
      ...
      if (existingSize) {
        ...
        return res
          .status(400)
          .json({ success: false, message: "Size already exists" });
      }
      ...
      if (size) {
        ...
        return res
          .status(200)
          .json({ success: true, message: "Size added successfully" });
      }
      ...
      return res
        .status(500)
        .json({ success: false, message: "Failed to add size" });
    }
  },
  ...
  deleteColor: async (req, res) => {
    ...
    try {
      ...
      if (productCount > 0) {
        ...
        return res.status(400).json({
          success: false,
          message: "Color is in use by some products",
        });
      } else {
  ...
  deleteSize: async (req, res) => {
    ...
    try {
      ...
      if (productUsingSize) {
        ...
        return res
          .status(400)
          .json({ success: false, message: "Size is in use by a product" });
      } else {
  ...
  deleteBrand: async (req, res) => {
    ...
    try {
      ...
      if (productsUsingBrand.length > 0) {
        ...
        return res.status(400).json({
          success: false,
          message: "Brand is in use by some products",
        });
      } else {
